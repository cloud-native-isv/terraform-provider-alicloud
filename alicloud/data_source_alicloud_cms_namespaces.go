...
func dataSourceAliCloudCmsNamespacesRead(d *schema.ResourceData, meta interface{}) error {
	...
	for {
		...
		if err != nil {
			return WrapErrorf(err, DataDefaultErrorMsg, "alicloud_cms_namespaces", action, AlibabaCloudSdkGoERROR)
		}
		...
		if err != nil {
			return WrapErrorf(err, FailedGetAttributeMsg, action, "$.DescribeHybridMonitorNamespace", response)
		}
		result, _ := resp.([]interface{})
		for _, v := range result {
			item := v.(map[string]interface{})
			if len(idsMap) > 0 {
				if _, ok := idsMap[fmt.Sprint(item["Namespace"])]; !ok {
					continue
				}
			}
	...
	for _, object := range objects {
		mapping := map[string]interface{}{
			...
			"id":            fmt.Sprint(object["Namespace"]),
			"namespace_id":  fmt.Sprint(object["Id"]),
			"namespace":     fmt.Sprint(object["Namespace"]),
			"specification": object["Detail"].(map[string]interface{})["Spec"],
		}
