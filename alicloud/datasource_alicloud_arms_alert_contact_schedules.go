...
func dataSourceAlicloudArmsAlertContactSchedulesRead(d *schema.ResourceData, meta interface{}) error {
	...
	for {
		...
		if err != nil {
			return WrapErrorf(err, DataDefaultErrorMsg, "alicloud_arms_alert_contact_schedules", action, AlibabaCloudSdkGoERROR)
		}
		resp, err := jsonpath.Get("$.PageBean.Schedules", response)
		if err != nil {
			return WrapErrorf(err, FailedGetAttributeMsg, action, "$.PageBean.Schedules", response)
		}
		result, _ := resp.([]interface{})
		for _, v := range result {
			item := v.(map[string]interface{})
			if scheduleNameRegex != nil && !scheduleNameRegex.MatchString(fmt.Sprint(item["ScheduleName"])) {
				continue
			}
			if len(idsMap) > 0 {
				if _, ok := idsMap[fmt.Sprint(item["ScheduleId"])]; !ok {
					continue
				}
			}
	...
	for _, object := range objects {
		mapping := map[string]interface{}{
			"id":             fmt.Sprint(object["ScheduleId"]),
			"schedule_id":    fmt.Sprint(object["ScheduleId"]),
			"schedule_name":  object["ScheduleName"],
			"description":    object["Description"],
			"alert_robot_id": fmt.Sprint(object["AlertRobotId"]),
			"state":          object["State"],
