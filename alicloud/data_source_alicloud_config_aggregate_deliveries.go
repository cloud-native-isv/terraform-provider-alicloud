...
func dataSourceAliCloudConfigAggregateDeliveries() *schema.Resource {
	return &schema.Resource{
		Read: dataSourceAliCloudConfigAggregateDeliveriesRead,
...
func dataSourceAliCloudConfigAggregateDeliveriesRead(d *schema.ResourceData, meta interface{}) error {
	...
	if err != nil {
		return WrapErrorf(err, DataDefaultErrorMsg, "alicloud_config_aggregate_deliveries", action, AlibabaCloudSdkGoERROR)
	}
	...
	for _, v := range result {
		...
		if len(idsMap) > 0 {
			...
			if _, ok := idsMap[fmt.Sprint(request["AggregatorId"], ":", item["DeliveryChannelId"])]; !ok {
				continue
			}
		}
	...
	for _, object := range objects {
		mapping := map[string]interface{}{
			...
			"aggregator_id":                          object["AggregatorId"],
