...
func (s *SlsService) DescribeSlsAlert(id string) (*aliyunSlsAPI.Alert, error) {
	if s.aliyunSlsAPI == nil {
	...
	alert, err := s.aliyunSlsAPI.GetAlert(projectName, alertName)
	if err != nil {
		if strings.Contains(err.Error(), "JobNotExist") {
			return nil, WrapErrorf(NotFoundErr("Alert", id), NotFoundMsg, "")
		}
		return nil, WrapErrorf(err, DefaultErrorMsg, id, "GetAlert", AlibabaCloudSdkGoERROR)
	}
...
func (s *SlsService) SlsAlertStateRefreshFunc(id string, field string, failStates []string) resource.StateRefreshFunc {
	return func() (interface{}, string, error) {
		alert, err := s.DescribeSlsAlert(id)
		if err != nil {
			if NotFoundError(err) {
				return alert, "", nil
			}
		...
		// Extract status from alert object based on field
		switch field {
		case "status":
			currentStatus = alert.Status
		case "state":
			currentStatus = alert.State
		case "name":
			currentStatus = alert.Name
		case "displayName":
			currentStatus = alert.DisplayName
		case "description":
			currentStatus = alert.Description
		default:
			// For other fields, convert to map and use jsonpath
			alertMap := make(map[string]interface{})
			alertMap["name"] = alert.Name
			alertMap["displayName"] = alert.DisplayName
			alertMap["description"] = alert.Description
			alertMap["state"] = alert.State
			alertMap["status"] = alert.Status
			alertMap["configuration"] = alert.Configuration
			alertMap["schedule"] = alert.Schedule
			alertMap["createTime"] = alert.CreateTime
			alertMap["lastModifyTime"] = alert.LastModifyTime

			v, jsonErr := jsonpath.Get(field, alertMap)
			if jsonErr != nil {
				return nil, "", WrapErrorf(jsonErr, FailedGetAttributeMsg, id, field)
			}
			currentStatus = fmt.Sprint(v)
		...
		for _, failState := range failStates {
			if currentStatus == failState {
				return alert, currentStatus, WrapError(Error(FailedToReachTargetStatus, currentStatus))
			}
		}
		return alert, currentStatus, nil
	}
}
...
func (s *SlsService) DescribeSlsScheduledSQL(id string) (*aliyunSlsAPI.ScheduledSQL, error) {
	...
	if err != nil {
		if strings.Contains(err.Error(), "JobNotExist") {
			return nil, WrapErrorf(NotFoundErr("ScheduledSQL", id), NotFoundMsg, "")
		}
		return nil, WrapErrorf(err, DefaultErrorMsg, id, "GetScheduledSQL", AlibabaCloudSdkGoERROR)
	}
...
func (s *SlsService) SlsScheduledSQLStateRefreshFunc(id string, field string, failStates []string) resource.StateRefreshFunc {
	return func() (interface{}, string, error) {
		scheduledSQL, err := s.DescribeSlsScheduledSQL(id)
		if err != nil {
			if NotFoundError(err) {
				return scheduledSQL, "", nil
			}
		...
		// Extract status from scheduledSQL object based on field
		switch field {
		case "status":
			currentStatus = scheduledSQL.Status
		case "name":
			currentStatus = scheduledSQL.Name
		case "displayName":
			currentStatus = scheduledSQL.DisplayName
		case "description":
			currentStatus = scheduledSQL.Description
		default:
			// For other fields, convert to map and use jsonpath
			scheduledSQLMap := make(map[string]interface{})
			scheduledSQLMap["name"] = scheduledSQL.Name
			scheduledSQLMap["displayName"] = scheduledSQL.DisplayName
			scheduledSQLMap["description"] = scheduledSQL.Description
			scheduledSQLMap["status"] = scheduledSQL.Status
			scheduledSQLMap["configuration"] = scheduledSQL.Configuration
			scheduledSQLMap["schedule"] = scheduledSQL.Schedule
			scheduledSQLMap["createTime"] = scheduledSQL.CreateTime
			scheduledSQLMap["lastModifyTime"] = scheduledSQL.LastModifyTime

			v, jsonErr := jsonpath.Get(field, scheduledSQLMap)
			if jsonErr != nil {
				return nil, "", WrapErrorf(jsonErr, FailedGetAttributeMsg, id, field)
			}
			currentStatus = fmt.Sprint(v)
		...
		for _, failState := range failStates {
			if currentStatus == failState {
				return scheduledSQL, currentStatus, WrapError(Error(FailedToReachTargetStatus, currentStatus))
			}
		}
		return scheduledSQL, currentStatus, nil
	}
}

// DescribeSlsEtl <<< Encapsulated get interface for Sls Etl.

func (s *SlsService) DescribeSlsEtl(id string) (*aliyunSlsAPI.ETL, error) {
	if s.aliyunSlsAPI == nil {
		return nil, fmt.Errorf("aliyunSlsAPI client is not initialized")
	}
	...
	etl, err := s.aliyunSlsAPI.GetETL(projectName, etlName)
	if err != nil {
		if strings.Contains(err.Error(), "JobNotExist") {
			return nil, WrapErrorf(NotFoundErr("Etl", id), NotFoundMsg, "")
		}
		return nil, WrapErrorf(err, DefaultErrorMsg, id, "GetETL", AlibabaCloudSdkGoERROR)
	}
...
func (s *SlsService) SlsEtlStateRefreshFunc(id string, field string, failStates []string) resource.StateRefreshFunc {
	return func() (interface{}, string, error) {
		...
		if strings.HasPrefix(field, "#") {
			...
			// Extract status from etl object based on field
			switch field {
			case "status":
				currentStatus = etl.Status
			case "name":
				currentStatus = etl.Name
			case "displayName":
				currentStatus = etl.DisplayName
			case "description":
				currentStatus = etl.Description
			default:
				// For other fields, convert to map and use jsonpath
				etlMap := make(map[string]interface{})
				etlMap["name"] = etl.Name
				etlMap["displayName"] = etl.DisplayName
				etlMap["description"] = etl.Description
				etlMap["status"] = etl.Status
				etlMap["configuration"] = etl.Configuration
				etlMap["schedule"] = etl.Schedule
				etlMap["createTime"] = etl.CreateTime

				v, jsonErr := jsonpath.Get(field, etlMap)
				if jsonErr != nil {
					return nil, "", WrapErrorf(jsonErr, FailedGetAttributeMsg, id, field)
				}
				currentStatus = fmt.Sprint(v)
		...
		for _, failState := range failStates {
			if currentStatus == failState {
				return etl, currentStatus, WrapError(Error(FailedToReachTargetStatus, currentStatus))
			}
		}
		return etl, currentStatus, nil
	}
}

// DescribeSlsOssExportSink <<< Encapsulated get interface for Sls OssExportSink.

func (s *SlsService) DescribeSlsOssExportSink(id string) (*aliyunSlsAPI.OSSExport, error) {
	if s.aliyunSlsAPI == nil {
		return nil, fmt.Errorf("aliyunSlsAPI client is not initialized")
	}
	...
	ossExport, err := s.aliyunSlsAPI.GetOSSExport(projectName, ossExportName)
	if err != nil {
		if strings.Contains(err.Error(), "JobNotExist") {
			return nil, WrapErrorf(NotFoundErr("OssExportSink", id), NotFoundMsg, "")
		}
		return nil, WrapErrorf(err, DefaultErrorMsg, id, "GetOSSExport", AlibabaCloudSdkGoERROR)
	}

	return ossExport, nil
}

func (s *SlsService) SlsOssExportSinkStateRefreshFunc(id string, field string, failStates []string) resource.StateRefreshFunc {
	return func() (interface{}, string, error) {
		ossExport, err := s.DescribeSlsOssExportSink(id)
		if err != nil {
			if NotFoundError(err) {
				return ossExport, "", nil
			}
		...
		if strings.HasPrefix(field, "#") {
			...
			// Extract status from ossExport object based on field
			switch field {
			case "status":
				currentStatus = ossExport.Status
			case "name":
				currentStatus = ossExport.Name
			case "displayName":
				currentStatus = ossExport.DisplayName
			case "description":
				currentStatus = ossExport.Description
			default:
				// For other fields, convert to map and use jsonpath
				ossExportMap := make(map[string]interface{})
				ossExportMap["name"] = ossExport.Name
				ossExportMap["displayName"] = ossExport.DisplayName
				ossExportMap["description"] = ossExport.Description
				ossExportMap["status"] = ossExport.Status
				ossExportMap["configuration"] = ossExport.Configuration
				ossExportMap["schedule"] = ossExport.Schedule
				ossExportMap["createTime"] = ossExport.CreateTime
				ossExportMap["lastModifyTime"] = ossExport.LastModifyTime

				v, jsonErr := jsonpath.Get(field, ossExportMap)
				if jsonErr != nil {
					return nil, "", WrapErrorf(jsonErr, FailedGetAttributeMsg, id, field)
				}
				currentStatus = fmt.Sprint(v)
		...
		for _, failState := range failStates {
			if currentStatus == failState {
				return ossExport, currentStatus, WrapError(Error(FailedToReachTargetStatus, currentStatus))
			}
		}
		return ossExport, currentStatus, nil
	}
}
