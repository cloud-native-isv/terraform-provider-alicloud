// filepath: /Users/liuqiming.lqm/project/cloud-native-isv/terraform-provider-alicloud/alicloud/resource_alicloud_flink_instance.go
package alicloud
...
func resourceAliCloudFlinkInstance() *schema.Resource {
	return &schema.Resource{
		Create: resourceAliCloudFlinkInstanceCreate,
		Read:   resourceAliCloudFlinkInstanceRead,
		Delete: resourceAliCloudFlinkInstanceDelete,
...
func resourceAliCloudFlinkInstanceCreate(d *schema.ResourceData, meta interface{}) error {
	...
	stateConf := resource.StateChangeConf{
		...
		Refresh:    flinkService.FlinkInstanceStateRefreshFunc(d.Id()),
	...
	return resourceAliCloudFlinkInstanceRead(d, meta)
}

func resourceAliCloudFlinkInstanceRead(d *schema.ResourceData, meta interface{}) error {
	client := meta.(*connectivity.AliyunClient)
	flinkService, err := NewFlinkService(client)
	if err != nil {
		return WrapError(err)
...
func resourceAliCloudFlinkInstanceDelete(d *schema.ResourceData, meta interface{}) error {
	client := meta.(*connectivity.AliyunClient)
	flinkService, err := NewFlinkService(client)
	if err != nil {
		return WrapError(err)
	...
	stateConf := BuildStateConf([]string{"DELETING", "RUNNING"}, []string{}, d.Timeout(schema.TimeoutDelete), 10*time.Second, flinkService.FlinkInstanceStateRefreshFunc(d.Id()))
	if _, err := stateConf.WaitForState(); err != nil {
