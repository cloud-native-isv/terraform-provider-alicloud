...
func dataSourceAliCloudResourceManagerDelegatedAdministratorsRead(d *schema.ResourceData, meta interface{}) error {
	...
	for {
		...
		if err != nil {
			return WrapErrorf(err, DataDefaultErrorMsg, "alicloud_resource_manager_delegated_administrators", action, AlibabaCloudSdkGoERROR)
		}
		...
		for _, v := range result {
			...
			if len(idsMap) > 0 {
				if _, ok := idsMap[fmt.Sprint(item["AccountId"], ":", item["ServicePrincipal"])]; !ok {
					continue
				}
			}
	...
	for _, object := range objects {
		mapping := map[string]interface{}{
			...
			"delegation_enabled_time": object["DelegationEnabledTime"],
			"id":                      fmt.Sprint(object["AccountId"], ":", object["ServicePrincipal"]),
			"service_principal":       fmt.Sprint(object["ServicePrincipal"]),
		}
	...
	if err := d.Set("administrators", s); err != nil {
		return WrapError(err)
	}
