...
func TestUnitAliCloudECSDedicatedHostCluster(t *testing.T) {
	...
	ReadMockResponse := map[string]interface{}{
		"DedicatedHostClusters": map[string]interface{}{
			"DedicatedHostCluster": []interface{}{
				map[string]interface{}{
					"DedicatedHostClusterName": "dedicated_host_cluster_name",
					"Description":              "description",
					"ZoneId":                   "zone_id",
	...
	responseMock := map[string]func(errorCode string) (map[string]interface{}, error){
		...
		"CreateNormal": func(errorCode string) (map[string]interface{}, error) {
			result := ReadMockResponse
			result["DedicatedHostClusterId"] = "MockDedicatedHostClusterId"
			return result, nil
		},
		...
		"ReadDescribeEcsDedicatedHostClusterNotFound": func(errorCode string) (map[string]interface{}, error) {
	...
	t.Run("CreateClientAbnormal", func(t *testing.T) {
		...
		err := resourceAliCloudEcsDedicatedHostClusterCreate(d, rawClient)
		patches.Reset()
		assert.NotNil(t, err)
	})
	...
	t.Run("CreateNormal", func(t *testing.T) {
		...
		err := resourceAliCloudEcsDedicatedHostClusterCreate(dCreate, rawClient)
		patches.Reset()
		assert.Nil(t, err)
	})
	...
	d.SetId("MockDedicatedHostClusterId")
	// Update
	t.Run("UpdateClientAbnormal", func(t *testing.T) {
		...
		err := resourceAliCloudEcsDedicatedHostClusterUpdate(d, rawClient)
		patches.Reset()
		assert.NotNil(t, err)
	})
	t.Run("UpdateModifyDedicatedHostClusterAttributeAbnormal", func(t *testing.T) {
		diff := terraform.NewInstanceDiff()
		for _, key := range []string{"dedicated_host_cluster_name", "description"} {
			switch p["alicloud_ecs_dedicated_host_cluster"].Schema[key].Type {
		...
		resourceData1, _ := schema.InternalMap(p["alicloud_ecs_dedicated_host_cluster"].Schema).Data(nil, diff)
		...
		err := resourceAliCloudEcsDedicatedHostClusterUpdate(resourceData1, rawClient)
		patches.Reset()
		assert.NotNil(t, err)
	})

	t.Run("UpdateModifyDedicatedHostClusterAttributeTagsAbnormal", func(t *testing.T) {
		diff := terraform.NewInstanceDiff()
		for _, key := range []string{"dedicated_host_cluster_name", "description", "tags"} {
			switch p["alicloud_ecs_dedicated_host_cluster"].Schema[key].Type {
			case schema.TypeString:
				diff.SetAttribute(key, &terraform.ResourceAttrDiff{Old: d.Get(key).(string), New: d.Get(key).(string) + "_update"})
			case schema.TypeBool:
	...
	t.Run("UpdateModifyDedicatedHostClusterAttributeNormal", func(t *testing.T) {
		diff := terraform.NewInstanceDiff()
		for _, key := range []string{"dedicated_host_cluster_name", "description"} {
			switch p["alicloud_ecs_dedicated_host_cluster"].Schema[key].Type {
			case schema.TypeString:
	...
	t.Run("DeleteClientAbnormal", func(t *testing.T) {
		...
		err := resourceAliCloudEcsDedicatedHostClusterDelete(d, rawClient)
		patches.Reset()
		assert.NotNil(t, err)
	})
	...
	//Read
	t.Run("ReadDescribeEcsDedicatedHostClusterNotFound", func(t *testing.T) {
		patcheDorequest := gomonkey.ApplyMethod(reflect.TypeOf(&client.Client{}), "DoRequest", func(_ *client.Client, _ *string, _ *string, _ *string, _ *string, _ *string, _ map[string]interface{}, _ map[string]interface{}, _ *util.RuntimeOptions) (map[string]interface{}, error) {
			NotFoundFlag := true
			noRetryFlag := false
			if NotFoundFlag {
				return responseMock["ReadDescribeEcsDedicatedHostClusterNotFound"]("")
			} else if noRetryFlag {
				return responseMock["NoRetryError"]("NoRetryError")
			}
			return responseMock["ReadNormal"]("")
		})
		err := resourceAliCloudEcsDedicatedHostClusterRead(d, rawClient)
