...
func resourceAliCloudConfigAggregateDeliveryCreate(d *schema.ResourceData, meta interface{}) error {
	...
	query["AggregatorId"] = d.Get("aggregator_id")
	...
	d.SetId(fmt.Sprintf("%v:%v", query["AggregatorId"], response["DeliveryChannelId"]))
...
func resourceAliCloudConfigAggregateDeliveryRead(d *schema.ResourceData, meta interface{}) error {
	...
	if objectRaw["AggregatorId"] != nil {
		d.Set("aggregator_id", objectRaw["AggregatorId"])
	}
	if objectRaw["DeliveryChannelId"] != nil {
		d.Set("delivery_channel_id", objectRaw["DeliveryChannelId"])
	}
