...
func (s *VpcPeerServiceV2) DescribeVpcPeerPeerConnection(id string) (object map[string]interface{}, err error) {
	...
	if err != nil {
		if IsExpectedErrors(err, []string{"ResourceNotFound.InstanceId"}) {
			return object, WrapErrorf(NotFoundErr("PeerConnection", id), NotFoundMsg, response)
		}
...
// DescribeVpcPeerPeerConnectionAccepter <<< Encapsulated get interface for VpcPeer PeerConnectionAccepter.

func (s *VpcPeerServiceV2) DescribeVpcPeerPeerConnectionAccepter(id string) (object map[string]interface{}, err error) {
	client := s.client
	var request map[string]interface{}
	var response map[string]interface{}
	var query map[string]interface{}
	...
	if err != nil {
		if IsExpectedErrors(err, []string{"ResourceNotFound.InstanceId"}) {
			return object, WrapErrorf(NotFoundErr("PeerConnectionAccepter", id), NotFoundMsg, response)
		}
		return object, WrapErrorf(err, DefaultErrorMsg, id, action, AlibabaCloudSdkGoERROR)
	}
...
// DescribeVpcPeerPeerConnectionAccepter >>> Encapsulated.
