...
Process `$ARGUMENTS` per the [User Input Protocol](skills/sdd-workflow/references/user-input-protocol.md). If empty, display list of all registered tools. If `tool_id` and natural-language hint conflict, stop and request correction.
...
For detailed tool type semantics, behavioral rules format, edge cases, and invocation preview format, see `skills/sdd-workflow/references/tool-definitions.md`.
